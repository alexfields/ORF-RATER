...
parser.add_argument('bamfiles',nargs='+',help='Path to transcriptome-aligned BAM file(s) to use for transcript filtering purposes')
...
parser.add_argument('--minlen',type=int,default=29,help='Minimum length of read to be considered when evaluating transcripts. ' cd \
                                                        'Also serves as the size of the segment when identifying multimapping positions. (Default: 29)')
parser.add_argument('--maxlen',type=int,default=30,help='Maximum length (inclusive) of read to be considered when evaluating transcripts. (Default: 30)')
parser.add_argument('-r','--minreads',type=int,default=64,help='Minimum number of reads demanded for each transcript (Default: 64)')
parser.add_argument('-f','--peakfrac',type=float,default=1./5,help='Maximum fraction of a transcript\'s reads coming from any one position (Default: 0.2)')
parser.add_argument('--pseudogenes',help='List of transcript IDs annotated as pseudogenes, one per line. To be removed if >PSEUDOFRAC fraction of reads mapping to each are multimappers.')
parser.add_argument('--pseudofrac',type=float,default=1./3,help='Maximum allowable multimapping fraction of an annotated pseudogene\'s reads. Ignored if list of pseudogenes is not provided. (Default: 0.333)')
parser.add_argument('--multiexcess',type=float,default=1./3,help='Maximum disparity in multimapping reads versus multimapping positions for any transcript (Default: 0.333)')
...
gnd = BAMGenomeArray(inbams,FivePrimeMapFactory(psite))
    # map to roughly the center of each read so that identical sequences that cross different splice sites (on different transcripts) still end up mapping to the same place
...
#    tlen = sum(int(x) for x in ls[10].strip(',').split(','))
#    if tlen > max_tlen:
#        max_tlen = tlen
#    tid_strlen = len(ls[3])
#    if tid_strlen > max_tid_strlen:
#        max_tid_strlen = tid_strlen
    ntids += 1
...
#itemsize_dict = {'tid':max_tid_strlen,
#                 'genpos':4,
#                 'seq':8,
#                 'reads':4}
#type_dict = {'tid':'a',
...
#    orig_store_name = seq_info_hdf_orig%(chrom,strand)
#    with pd.get_store(orig_store_name,mode='w',complevel=1,complib='blosc') as tid_info_store:
#        datbuffer = buffered_dataframe_hdf_writer(itemsize_dict,
#                                                  type_dict,
#                                                  chunksize=max_tlen,
#                                                  estimated_total_rows=len(genome[chrom]),
#                                                  hdf_store=tid_info_store,
#                                                  df_name='tid_seq_info',
#                                                  data_columns=True)
    tid_seq_info = []
    tid_summary = pd.DataFrame({'chrom':chrom,'strand':strand,'n_psite':-1,'n_reads':-1,'peak_reads':-1,'dropped':''},index=pd.Index(bedlinedict[(chrom,strand)].keys(),name='tid'))
...
#                    datbuffer.add_items({'tid':tid,
#                                         'genpos':curr_pos_list[psite:n_psite+psite],
#                                         'seq':[(int(curr_seq[i:i+fpsize],4) if 'N' not in curr_seq[i:i+fpsize] else -1) for i in xrange(n_psite)],
#                                         'reads':curr_counts},
#                                         n_psite)
...
#        datbuffer.flush()
    if tid_seq_info:  # don't bother saving anything if there's nothing to save
        pd.concat(tid_seq_info,ignore_index=True).to_hdf(seq_info_hdf%(chrom,strand),'tid_seq_info',format='t',data_columns=True,complevel=1,complib='blosc')
...
# This divides up the job naturally - because a sequence beginning with 'A' cannot multimap with one beginning with 'T' etc
...
def _find_mm_in_range(partnum):
    ...
    seq_df.to_hdf(outname%partnum,'tid_seq_mm',format='t',data_columns=True)  # save the result to avoid having to recalculate (after dropping pseudogenes)
#    seq_df.to_hdf(outname_orig%partnum,'tid_seq_mm',format='t',data_columns=True)  # save the result to avoid having to recalculate (after dropping pseudogenes)
...
tid_info = pd.concat((tid_summary[['chrom','strand','n_psite','n_reads']],
    ...
    .fillna({'mm_psite':0,'mm_reads':0})  # if a transcript isn't listed in mm_df_res, it doesn't have any multimapping positions
...
if pseudos.size>0:
    ...
    tid_info = pd.concat((tid_summary[['chrom','strand','n_psite','n_reads']],
                          mm_df_res.rename(columns={'genpos':'mm_psite','reads':'mm_reads'})),axis=1) \
        .fillna({'mm_psite':0,'mm_reads':0})  # if a transcript isn't listed in mm_df_res, it doesn't have any multimapping positions, or it has been dropped
    tid_info['reads_mm_frac'] = tid_info['mm_reads']/tid_info['n_reads']
    tid_info['psite_mm_frac'] = tid_info['mm_psite']/tid_info['n_psite']
